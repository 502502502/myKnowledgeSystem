...
拿到服务端下一次预期接收的序列号，作为 RST 报文的序列号



## 62、四次挥手中收到乱序的 FIN 包会如何处理？

在四次挥手中，收到乱序的FIN包时，TCP协议栈会根据TCP的可靠性机制进行处理。

1. 接收乱序FIN包：当接收方的TCP协议栈接收到乱序的FIN包时，它会首先检查包的序列号，并将其缓存起来。
2. 等待缺失的数据段：TCP协议栈会继续等待后续传输过程中的数据段，直到收到所有缺失的数据段。
3. 接收完整的数据段：当接收方的TCP协议栈收到缺失的数据段后，它会按照序列号的顺序重新排序这些数据段，并将它们缓存起来。
4. 回复ACK包：一旦接收方成功接收并重新排序了所有数据段，它会发送一个ACK包作为回复，确认已经接收到了乱序FIN包以及之前的数据段。
5. 进行正常的挥手流程：完成对乱序FIN包的处理后，接收方会继续发送自己的FIN包，触发正常的挥手流程。
